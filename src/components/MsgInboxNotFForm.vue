  ...
  <form @submit.prevent="handleSubmit" class="space-y-4">
    ...
    <div>
      ...
      <input
        ...
        class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:border-indigo-500 focus:border-indigo-500 sm:text-sm"
      />
    </div>
...
export default {
  ...
  methods: {
    handleSubmit() {
      // If status_flag is not set but this is an update, allow submission
      if (!this.formCopy.status_flag && this.formCopy.em_alert_id) {
        this.formCopy.status_flag = 'Inactive'; // Default to Inactive if not set
      }
